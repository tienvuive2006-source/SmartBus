...
public class EmailService {
    ...
    public void sendBookingConfirmation(Booking booking) {
        CompletableFuture.runAsync(() -> {
            try {
                ...
                String statusForQr = "CASH".equalsIgnoreCase(booking.getPaymentMethod())
                ...
                // Generate the QR Code image (500x500 pixels for crispness) and convert to
                // Base64
                ...
                if (response.statusCode() >= 200 && response.statusCode() < 300) {
                    System.out.println(
                            "📧 [EmailService] Gửi email qua Brevo thành công cho: " + booking.getCustomerEmail());
                } else {
                    System.err.println(
                            "❌ [EmailService] Brevo API lỗi: " + response.statusCode() + " - " + response.body());
                }
            } catch (Exception e) {
    ...
    private String buildHtmlTemplate(Booking booking) {
        String departureDateStr = booking.getTrip().getDepartureDate() != null ? booking.getTrip().getDepartureDate()
                : "Chưa xác định";
        ...
        String paymentMethodStr = "CASH".equalsIgnoreCase(booking.getPaymentMethod())
                ...
                : ("QR".equalsIgnoreCase(booking.getPaymentMethod()) ? "Chuyển khoản QR Code"
        ...
        return "<!DOCTYPE html>" +
                ...
                "                    <div class='ticket-cell ticket-value'>" + booking.getTrip().getCompanyName()
                + "</div>" +
                "                </div>" +
                "                <div class='ticket-row'>" +
                ...
                "                    <div class='ticket-cell ticket-value' style='color: #0f172a;'>"
                + booking.getTrip().getDeparturePoint() + " ➔ " + booking.getTrip().getArrivalPoint() + "</div>" +
                "                </div>" +
                "                <div class='ticket-row'>" +
                ...
                "                    <div class='ticket-cell ticket-value' style='color: #e11d48;'>"
                + booking.getTrip().getDepartureTime() + "</div>" +
                "                </div>" +
                "                <div class='ticket-row'>" +
                "                    <div class='ticket-cell ticket-label'>Số ghế đặt:</div>" +
                "                    <div class='ticket-cell ticket-value' style='background: #cbd5e1; padding: 2px 8px; border-radius: 4px; display: inline-block; font-weight: bold;'>"
                + seatsJoined + "</div>" +
                "                </div>" +
                "                <div class='ticket-row'>" +
                "                    <div class='ticket-cell ticket-label'>Tổng tiền:</div>" +
                "                    <div class='ticket-cell ticket-value' style='color: #10b981; font-size: 16px;'>"
                ...
                "                    <div class='ticket-cell ticket-value' style='font-weight: bold;'>"
                + paymentMethodStr + "</div>" +
                "                </div>" +
                "            </div>" +
                "            " +
